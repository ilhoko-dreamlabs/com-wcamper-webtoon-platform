...
| ADR-028 | GitHub `origin` is the project release remote | Approved | Repository push and production release work proceed through GitHub and Vercel only |
| ADR-029 | Obsolete non-GitHub handoff artifacts must be removed from the current plan | Approved | Obsolete CI, handoff, request, report, and package command artifacts are removed to avoid release-path confusion |
| ADR-030 | GitHub push/PR work requires explicit approval before remote action | Approved | Remote push, PR creation, deploy, DB migration, secret rotation, DNS, CDN, and public URL changes remain separate approvals |

...
`webtoon.wcamper.com` 캠핑 웹툰 MVP입니다. 1차 목표는 독자가 볼 수 있는 정적 연재 사이트와 AI 웹툰 제작 worker가 사용할 제한된 회차 작업공간을 함께 마련하는 것입니다. 다음 백엔드 단계에서는 작가가 GPT 등 자신이 쓰는 AI 도구에서 제작한 웹툰을 API, MCP, GPT Actions로 제출할 수 있게 하는 구조를 우선합니다.
...
- `BD크루 캠핑일지` 공개 연재
- 공개 회차 2개: 1화 `첫 번째 밤, 타프 아래에서`, 2화 `비 예보와 타프 각도`
- 3화 `철수 전 아침 커피` 제작중 표시
- 메인
- 작품 목록
- 작품 상세
- 회차 목록

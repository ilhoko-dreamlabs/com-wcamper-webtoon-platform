...
async function runPlatformSchemaStatements() {
  const statements = [
    ...
    `alter table publication_snapshots add column if not exists snapshot_type text not null default 'CATALOG'`,
    `alter table publication_snapshots add column if not exists target_id text`,
    `alter table publication_snapshots add column if not exists source_hash text`,
    `alter table publication_snapshots add column if not exists output_path text`,
    `alter table publication_snapshots add column if not exists metadata jsonb not null default '{}'::jsonb`,
    ...
    `create index if not exists static_artifacts_snapshot_idx on static_artifacts (snapshot_id, created_at desc)`,
    ...
    `create index if not exists publication_releases_env_created_idx on publication_releases (environment, created_at desc)`,
    ...
    `create index if not exists authoring_imports_author_created_idx on authoring_imports (author_id, created_at desc)`,
    ...
    `create index if not exists authoring_import_events_import_created_idx on authoring_import_events (import_id, created_at desc)`,
